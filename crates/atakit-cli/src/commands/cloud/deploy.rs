...
use super::{
    collect_unmeasured_tar, ensure_cloud_image, parse_metadata, resolve_image, resolve_workload,
    ...
    match targets.len() {
        ...
        _ => {
            ...
            if !upload_pairs.is_empty() {
                ...
                {
                    eprintln!("  {} {} -> {}", "→".dimmed(), image_ref, provider_name,);
                    ensure_cloud_image(
						image_ref,
						provider_name,
						provider_config,
...
async fn run_one(args: DeployArgs, env: &Env, config: &Config, verbose: bool) -> Result<()> {
    ...
    if unmeasured_tar.is_some() {
        ...
        eprintln!(
            ...
            if unmeasured_data_paths.len() == 1 {
                ""
            } else {
                "s"
            },
                ...
                if matches!(
                    ...
                    if let Some(ref az) = state.resources.azure {
                        ...
                        eprintln!(
                            "  {:<12}{}",
                            "RG:".dimmed(),
                            az.resource_group.as_deref().unwrap_or("-")
                        );
                        eprintln!();
                    }
    ...
    match provider_config.platform {
        ...
        PlatformKind::Azure => {
            ...
            eprintln!("    {}:", "Serial console".dimmed());
            eprintln!(
                "      az serial-console connect --name {instance_name} --resource-group {}",
                names.resource_group
            );
            eprintln!();
            eprintln!("    {}:", "SSH".dimmed());
            eprintln!(
                "      az ssh vm --name {instance_name} --resource-group {}",
                names.resource_group
            );
        }
        PlatformKind::Aws => {
            ...
            eprintln!("    {}:", "Serial console".dimmed());
            eprintln!(
                "      aws ec2 get-console-output --region {region} --instance-id {vm_id} --latest"
            );
            eprintln!();
            eprintln!("    {}:", "SSH".dimmed());
...
fn parse_source(source: &str, v: Option<&str>) -> Result<Option<u64>> {
    match v {
        ...
        Some(s) => parse_size_gb(s).map(Some).ok_or_else(|| {
            anyhow::anyhow!(
                "invalid boot disk size {s:?} from {source} (expected e.g. \"100GB\", \"1TB\")"
            )
        }),
    }
}
...
fn resolve_boot_disk_size(
    ...
    let effective = cli_gb
        .or(target_gb)
        .or(workload_gb)
        .unwrap_or(DEFAULT_BOOT_DISK_GB);
...
mod tests {
    ...
    fn cli_below_workload_minimum_errors() {
        let err = resolve_boot_disk_size(Some("10GB"), None, Some("50GB"))
            .unwrap_err()
            .to_string();
    ...
    fn target_below_workload_minimum_errors() {
        let err = resolve_boot_disk_size(None, Some("10GB"), Some("50GB"))
            .unwrap_err()
            .to_string();
    ...
    fn below_hard_floor_errors_from_cli() {
        let err = resolve_boot_disk_size(Some("1GB"), None, None)
            .unwrap_err()
            .to_string();
    ...
    fn below_hard_floor_errors_from_workload() {
        let err = resolve_boot_disk_size(None, None, Some("1GB"))
            .unwrap_err()
            .to_string();
        assert!(err.contains("2GB"), "{err}");
    }
    ...
    fn bad_format_names_the_source() {
        let err = resolve_boot_disk_size(Some("garbage"), None, None)
            .unwrap_err()
            .to_string();
        assert!(err.contains("--boot-disk-size"), "{err}");
        let err = resolve_boot_disk_size(None, Some("garbage"), None)
            .unwrap_err()
            .to_string();
        assert!(err.contains("target.boot_disk_size"), "{err}");
        let err = resolve_boot_disk_size(None, None, Some("garbage"))
            .unwrap_err()
            .to_string();
