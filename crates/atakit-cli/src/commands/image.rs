...
use atakit_core::Env;
use atakit_image::{
    DEFAULT_REPO, ImageRef, ImageStore, LsArgs, Platform, PullArgs, Release, ReleaseStatus,
    ReleasesClient, RmArgs,
};
use owo_colors::OwoColorize;

use crate::progress::IndicatifReporter;

pub async fn run_ls(args: LsArgs, env: &Env) -> Result<()> {
...
pub async fn run_pull(args: PullArgs, env: &Env) -> Result<()> {
...
pub async fn run_rm(args: RmArgs, env: &Env) -> Result<()> {
    let store = ImageStore::new(&env.image_dir);
