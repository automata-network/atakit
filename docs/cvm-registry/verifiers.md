...
// DCAP output parsing
uint256 constant DCAP_QUOTE_BODY_OFFSET = 11;  // header: 2+2+1+6 bytes
uint256 constant DCAP_MIN_OUTPUT_LEN = 584;     // TD10_QUOTE_BODY_SIZE (520 + 64)
...
// SNP
uint256 constant SNP_REPORT_DATA_OFFSET = 0x50;  // 80 decimal
uint256 constant SNP_MIN_REPORT_LEN = 144;       // 80 + 64
```
...
3. Return quote body as `reportData` in result

**SNP (AMD) flow**:
1. ZK only (Solidity backend reverts with `UnsupportedBackendType`): `snpAttestation.verifyAndAttestWithZKProof(output, zkCoprocessor, proofBytes)`
2. Validate `journal.result == VerificationResult.Success`
3. Extract attestation report (`extractSnpAttestationReport` -- validates min length, returns raw report)
4. Return attestation report as `reportData` in result
...
NOTE: `extractDcapQuoteBody` uses inline assembly for byte extraction. TeeVerifier does NOT import LibBytes or BytesUtils -- all byte manipulation is done via raw assembly.
...
| `UnsupportedBackendType()` | Invalid verification backend (e.g. Solidity for SNP) |
| `InvalidTeeReport()` | Verification returned invalid, or report too short |
...
#### ES256 (typeId = 2)
1. Validate key is 65 bytes starting with `0x04`
2. Extract x, y coordinates from SEC1 uncompressed point
...
3. Call `ECDSA.tryRecoverCalldata(hash, signature)` to recover signer address
...
**File**: `src/bases/TpmVerifier.sol` (196 lines)
...
2. Validate `verificationBackendType == Solidity`
3. Decode `TpmCertifyReport` from `tpmReport.data`
4. Validate CLEAR bits on `tpmtPublic[4:8]` via `_validateClearBits` -- checks `TPMA_OBJECT_REQUIRED_CLEAR` BEFORE the library call
5. Convert `PublicIdentity` -> `CertPubkey`
6. Call `tpmAttestation.verifyTpmKeyCertification(tpm2bAttest, tpmSignature, tpmtPublic, akCertPubkey, TPMA_OBJECT_REQUIRED_SET)` -- the library validates SET bits
7. Convert certified key (`CertPubkey`) back to `PublicIdentity`
8. Compute fingerprint of certified key
9. Return: `{ valid: true, certifiedKey, certifiedKeyFingerprint }`
...
**File**: `src/bases/AkCollateralVerifier.sol` (190 lines)
**Inheritance**: `TpmBase` (abstract)
...
3. Construct `CertPubkey` via `LibX509.newRsaPubkey(nBytes, eBytes)`, convert to `PublicIdentity`
...
**File**: `src/bases/TpmBase.sol` (29 lines)
