...
**File**: `src/SessionRegistry.sol` (1,312 lines)
...
Actions:
  ...
  - Platform-specific binding:
    Azure (AzureAkPubJson):
      ...
      NOTE: Uses DCAP report data extraction (TDX-specific). Azure SNP binding not implemented.
    GCP (GcpCertChain) + TDX:
      - Extract UUID (16 bytes) from quoteBody at offset 520
      - Verify RTMR3 (at offset 472) == sha384(bytes48(0) || sha384(UUID))
      - Compute expectedPcr15 = sha256(bytes32(0) || sha256(UUID))
...
| `PCRNotFound()` | Required PCR not in quote |
