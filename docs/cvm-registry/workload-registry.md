...
**File**: `src/WorkloadRegistry.sol` (304 lines)
**Interface**: `src/interfaces/registries/IWorkloadRegistry.sol`
**Role**: Manages application measurement policies (typically PCR 20-23; only `pcrIndex < 24` is enforced)
...
- `spec.ttl` is consumed later by `SessionRegistry`; `ttl == 0` means "use DEFAULT_CVM_TTL (30 days)"
...
|---|---|---|
| PCR range | Typically platform PCRs | Typically workload PCRs |
