...
- Paths are written to `manifest.toml` so the CVM agent knows what to expect
...
2. Extract to `<WorkloadTempDir>/unmeasured-data/` (bind-mounted into the container at `/app/unmeasured-data/`)
3. Verify the directory structure matches what's declared in `manifest.toml`
...
- **PCR 23:** Workload measurement - SHA-256 of `manifest.toml`. Generated by `workload info` / `workload inspect`.
...
5. **Unmeasured-data tar layout.** Strip `./` prefix, preserve relative directory structure. `./runtime-data/key.pem` becomes `runtime-data/key.pem` in the tar. Agent extracts into `<WorkloadTempDir>/unmeasured-data/`, so it ends up at `<WorkloadTempDir>/unmeasured-data/runtime-data/key.pem` (bind-mounted into the container at `/app/unmeasured-data/runtime-data/key.pem`). Same convention as measured-data staging in `.atawl`.
